...
// ── テーブル形状ごとのバランスよい配置生成 ──
function generateLayout(tableType, tableCount, seatsPerTable) {
  const tables = [], seats = [];
  let sid = 1;

  if (tableType === 'long') {
    // 長テーブル：両サイドに均等配置
    const side1 = Math.ceil(seatsPerTable / 2);
    const side2 = Math.floor(seatsPerTable / 2);
    const tableW = Math.max(TW, side1 * (SW + 6));
    const COLS = tableCount <= 2 ? tableCount : Math.ceil(Math.sqrt(tableCount));
    ...
    for (let t = 0; t < tableCount; t++) {
      ...
      // 上側
      const sp1 = tableW / side1;
      for (let i = 0; i < side1; i++)
        seats.push({ id: sid++, x: tx + sp1*i + sp1/2 - SW/2, y: ty });
      // 下側
      const sp2 = tableW / Math.max(side2, 1);
      for (let i = 0; i < side2; i++)
        seats.push({ id: sid++, x: tx + sp2*i + sp2/2 - SW/2, y: ty + SH + 10 + TH + 10 });
    }
  }

  else if (tableType === 'round') {
    // 丸テーブル：円周上に均等配置
    const R = Math.max(38, seatsPerTable * 14); // 人数に応じて半径調整
    const SEAT_R = R + SW + 6;
    const COLS = tableCount <= 3 ? tableCount : Math.ceil(Math.sqrt(tableCount * 1.3));
    const GAP = SEAT_R * 2 + 50;
    for (let t = 0; t < tableCount; t++) {
      const col = t % COLS, row = Math.floor(t / COLS);
    ...
    // 島テーブル：4辺に均等配置
    const perSide = Math.ceil(seatsPerTable / 4);
    const tableW = Math.max(TW, perSide * (SW + 8));
    const tableH = Math.max(TH, perSide * (SH + 8));
    const COLS = tableCount <= 3 ? tableCount : Math.ceil(Math.sqrt(tableCount * 1.3));
    const GAP_X = tableW + SW * 2 + 60, GAP_Y = tableH + SH * 2 + 60;
    for (let t = 0; t < tableCount; t++) {
      const col = t % COLS, row = Math.floor(t / COLS);
      const tx = 30 + col * GAP_X, ty = 40 + row * GAP_Y;
      tables.push({ id: t, type: 'island', x: tx, y: ty + SH + 8, w: tableW, h: tableH });
      let remaining = seatsPerTable;
      // 上
      const top = Math.ceil(remaining / 4); const sp_t = tableW / top;
      for (let i = 0; i < top && sid <= sid + remaining; i++) seats.push({ id: sid++, x: tx + sp_t*i + sp_t/2 - SW/2, y: ty });
      remaining -= top;
      // 下
      const bot = Math.ceil(remaining / 3); const sp_b = tableW / Math.max(bot,1);
      for (let i = 0; i < bot; i++) seats.push({ id: sid++, x: tx + sp_b*i + sp_b/2 - SW/2, y: ty + SH + 8 + tableH + 8 });
      remaining -= bot;
      // 左
      const left = Math.ceil(remaining / 2); const sp_l = tableH / Math.max(left,1);
      for (let i = 0; i < left; i++) seats.push({ id: sid++, x: tx - SW - 8, y: ty + SH + 8 + sp_l*i + sp_l/2 - SH/2 });
      remaining -= left;
      // 右
      const sp_r = tableH / Math.max(remaining,1);
      for (let i = 0; i < remaining; i++) seats.push({ id: sid++, x: tx + tableW + 8, y: ty + SH + 8 + sp_r*i + sp_r/2 - SH/2 });
    }
  }

  else if (tableType === 'u') {
    // コの字：全テーブルを1つのコの字として配置
    const total = tableCount * seatsPerTable;
    const top = Math.ceil(total / 3), left = Math.floor((total - top) / 2), right = total - top - left;
    const tableW = Math.max(200, top * (SW + 6));
    const tableH = Math.max(160, Math.max(left, right) * (SH + 8));
    const ox = 50, oy = 50;
    ...
    const sp_t = tableW / top;
    for (let i = 0; i < top; i++) seats.push({ id: sid++, x: ox + sp_t*i + sp_t/2 - SW/2, y: oy - SH - 8 });
    const sp_l = tableH / Math.max(left,1);
    for (let i = 0; i < left; i++) seats.push({ id: sid++, x: ox - 16 - SW - 8, y: oy + 16 + sp_l*i + sp_l/2 - SH/2 });
    const sp_r = tableH / Math.max(right,1);
    for (let i = 0; i < right; i++) seats.push({ id: sid++, x: ox + tableW + 16 + 8, y: oy + 16 + sp_r*i + sp_r/2 - SH/2 });
  }
...
function LayoutEditor({ tables, seats, setTables, setSeats, people }) {
  ...
  function addTable(type) {
    ...
    const id=Date.now(), count=Math.min(type==='long'?8:type==='round'?6:4, people-totalSeats);
    let sid=totalSeats+1; const ns=[];
    if(type==='long'){const tx=60,ty=SH+10+50;setTables(p=>[...p,{id,type:'long',x:tx,y:ty,w:TW,h:TH}]);const sp=TW/4;for(let s=0;s<2;s++)for(let p=0;p<4&&ns.length<count;p++)ns.push({id:sid++,x:tx+sp*p+sp/2-SW/2,y:s===0?50:ty+TH+10});}
    else if(type==='round'){const R=40,cx=R+SW+30,cy=R+SW+30;setTables(p=>[...p,{id,type:'round',x:cx,y:cy,r:R}]);for(let i=0;i<count;i++){const a=(i/6)*Math.PI*2-Math.PI/2;ns.push({id:sid++,x:cx+Math.cos(a)*(R+SW+8)-SW/2,y:cy+Math.sin(a)*(R+SW+8)-SH/2});}}
    else{const tx=60,ty=SH+8+50;setTables(p=>[...p,{id,type:'island',x:tx,y:ty,w:TW,h:TH}]);const p4=[[tx+TW/4-SW/2,50],[tx+3*TW/4-SW/2,50],[tx+TW/4-SW/2,ty+TH+8],[tx+3*TW/4-SW/2,ty+TH+8]];for(let p=0;p<count;p++)ns.push({id:sid++,x:p4[p][0],y:p4[p][1]});}
    let s=1;setSeats([...seats,...ns].map(x=>({...x,id:s++})));
  }
  function addSeat(){if(totalSeats>=people)return;let s=1;setSeats([...seats,{id:0,x:60,y:60}].map(x=>({...x,id:s++})));}
  function deleteSelected(){const ns=seats.filter(s=>!selSeats.has(s.id));const nt=tables.filter(t=>!selTables.has(t.id));let s=1;setSeats(ns.map(x=>({...x,id:s++})));setTables(nt);setSelSeats(new Set());setSelTables(new Set());}
  ...
  return (
    <div>
      <div style={{marginBottom:8,padding:'8px 10px',background:'#f0f4ff',borderRadius:10,border:'1.5px solid #e8eaf6'}}>
        <div style={{fontSize:11,color:'#90a4ae',marginBottom:6,fontWeight:700}}>プリセットから読み込む</div>
        <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
          {[['long','⬛ 長'],['round','⭕ 丸'],['u','🔲 コの字'],['island','🟦 島']].map(([type,label])=>(
            <button key={type} onClick={()=>{if(totalSeats>0&&!window.confirm('現在の配置をリセットしますか？'))return;const{tables:pt,seats:ps}=generateLayout(type,1,Math.min(8,people));setTables(pt);setSeats(ps);setSelSeats(new Set());setSelTables(new Set());}}
              style={{padding:'5px 10px',borderRadius:8,fontSize:12,fontWeight:700,background:'#fff',color:'#5c6bc0',border:'1.5px solid #c5cae9'}}>{label}</button>
          ))}
        </div>
      </div>
      <div style={{display:'flex',gap:6,marginBottom:6,flexWrap:'wrap',alignItems:'center'}}>
        ...
        {[['long','⬛ 長'],['round','⭕ 丸'],['island','🟦 島']].map(([type,label])=>(
          <button key={type} onClick={()=>addTable(type)} disabled={totalSeats>=people} style={{padding:'5px 9px',borderRadius:8,fontSize:12,fontWeight:700,background:totalSeats>=people?'#f5f5f5':'#e8eaf6',color:totalSeats>=people?'#bdbdbd':'#5c6bc0',border:'1.5px solid #c5cae9'}}>{label}</button>
        ))}
        <button onClick={addSeat} disabled={totalSeats>=people} style={{padding:'5px 9px',borderRadius:8,fontSize:12,fontWeight:700,background:totalSeats>=people?'#f5f5f5':'#e8f5e9',color:totalSeats>=people?'#bdbdbd':'#43a047',border:'1.5px solid #c8e6c9'}}>＋席</button>
        <div style={{marginLeft:'auto',fontSize:12,fontWeight:700,color:totalSeats===people?'#43a047':'#f57c00'}}>{totalSeats}/{people}席</div>
      </div>
      <div style={{marginBottom:8,display:'flex',alignItems:'center',gap:8}}>
        <button onClick={()=>{setMultiMode(m=>!m);if(multiMode){setSelSeats(new Set());setSelTables(new Set());}}} style={{padding:'6px 14px',borderRadius:20,fontSize:12,fontWeight:700,background:multiMode?'#3949ab':'#f0f4ff',color:multiMode?'#fff':'#7986cb',border:`2px solid ${multiMode?'#3949ab':'#c5cae9'}`}}>
      ...
      <div style={{borderRadius:12,border:'2px solid #c5cae9',background:'#f8f9ff',overflow:'scroll',maxHeight:420,WebkitOverflowScrolling:'touch'}}>
        <svg ref={svgRef} width={SVG_W} height={SVG_H} style={{display:'block',cursor:dragging?'grabbing':'default',touchAction:dragging?'none':'auto'}}
          ...
          {tables.length===0&&seats.length===0&&<text x={SVG_W/2} y={SVG_H/2} textAnchor="middle" fill="#c5cae9" fontSize={14}>上のボタンでテーブルを追加</text>}
          {tables.map(t=><TableBodySVG key={t.id} table={t} selected={selTables.has(t.id)} onPointerDown={e=>{e.stopPropagation();const{x,y}=svgCoords(e);const multi=multiMode||e.shiftKey;if(multi){const n=new Set(selTables);n.has(t.id)?n.delete(t.id):n.add(t.id);setSelTables(n);}else if(!selTables.has(t.id)){setSelTables(new Set([t.id]));setSelSeats(new Set());}setDragging({startX:x,startY:y});setDragStart({seats:seats.map(s=>({...s})),tables:tables.map(tb=>({...tb}))});e.currentTarget.closest('svg').setPointerCapture(e.pointerId);}}/>)}
          {seats.map(s=><SeatSVG key={s.id} seat={s} assignment={null} highlight={null} selected={selSeats.has(s.id)} onPointerDown={(e,id)=>{const{x,y}=svgCoords(e);const multi=multiMode||e.shiftKey;if(multi){const n=new Set(selSeats);n.has(id)?n.delete(id):n.add(id);setSelSeats(n);}else if(!selSeats.has(id)){setSelSeats(new Set([id]));setSelTables(new Set());}setDragging({startX:x,startY:y});setDragStart({seats:seats.map(s=>({...s})),tables:tables.map(t=>({...t}))});}}/>)}
        </svg>
      </div>
      <div style={{marginTop:8,background:'#f0f4ff',borderRadius:12,padding:'8px 12px',border:'1.5px solid #e8eaf6',minHeight:50}}>
        {!hasSel&&<div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}><span style={{color:'#b0bec5',fontSize:12}}>タップで選択</span>{(seats.length>0||tables.length>0)&&<button onClick={selectAll} style={{...sBtn,background:'#e8eaf6',color:'#5c6bc0'}}>全選択</button>}</div>}
...
export default function App() {
  ...
  const totalPresetSeats = tableCount * seatsPerTable;
  const presetData = useMemo(() => generateLayout(tableType, tableCount, seatsPerTable), [tableType, tableCount, seatsPerTable]);
  ...
  // プリセット：席数が人数と一致しているかチェック
  const presetOk = totalPresetSeats === people;
  const canStart = layoutMode==='preset' ? presetOk : totalCustomSeats===people;
  ...
  function spin() {
    ...
    function tick() {
      ...
      count++;
      if(count>=total){const picked=remainingSeats[(count-1)%remainingSeats.length];setDisplaySeat(picked);setFinalSeat(picked);setSpinning(false);}
      else{const p=count/total;interval=p>0.6?80+(p-0.6)*600:80;setTimeout(tick,interval);}
    }
        ...
        <div style={C.page}>
              ...
              <div>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:16}}>
                  {[{id:'long',icon:'⬛',name:'長テーブル'},{id:'round',icon:'⭕',name:'丸テーブル'},{id:'u',icon:'🔲',name:'コの字'},{id:'island',icon:'🟦',name:'島テーブル'}].map(p=>(
                    <button key={p.id} onClick={()=>setTableType(p.id)}
                      style={{display:'flex',alignItems:'center',gap:8,padding:'10px 12px',borderRadius:12,fontFamily:'inherit',
                        background:tableType===p.id?'#dde3fa':'#f8f9ff',border:`2px solid ${tableType===p.id?'#5c6bc0':'#e8eaf6'}`}}>
                ...
                {tableType !== 'u' && (
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:12}}>
                    <div style={{background:'#f8f9ff',borderRadius:12,padding:'12px'}}>
                      <div style={{fontSize:11,color:'#90a4ae',marginBottom:6}}>テーブル数</div>
                      <div style={{display:'flex',alignItems:'center',gap:8}}>
                        <button onClick={()=>setTableCount(c=>Math.max(1,c-1))} style={{width:32,height:32,borderRadius:8,background:'#e8eaf6',color:'#5c6bc0',fontSize:18,fontWeight:700}}>−</button>
                        <span style={{fontFamily:"'Nunito',sans-serif",fontWeight:900,fontSize:28,color:'#3949ab',minWidth:32,textAlign:'center'}}>{tableCount}</span>
                        <button onClick={()=>setTableCount(c=>Math.min(10,c+1))} style={{width:32,height:32,borderRadius:8,background:'#e8eaf6',color:'#5c6bc0',fontSize:18,fontWeight:700}}>＋</button>
                      </div>
                      <div style={{fontSize:10,color:'#b0bec5',marginTop:4}}>最大10テーブル</div>
                    </div>
                    <div style={{background:'#f8f9ff',borderRadius:12,padding:'12px'}}>
                      <div style={{fontSize:11,color:'#90a4ae',marginBottom:6}}>1テーブルあたり</div>
                      <div style={{display:'flex',alignItems:'center',gap:8}}>
                        <button onClick={()=>setSeatsPerTable(s=>Math.max(2,s-1))} style={{width:32,height:32,borderRadius:8,background:'#e8eaf6',color:'#5c6bc0',fontSize:18,fontWeight:700}}>−</button>
                        <span style={{fontFamily:"'Nunito',sans-serif",fontWeight:900,fontSize:28,color:'#3949ab',minWidth:32,textAlign:'center'}}>{seatsPerTable}</span>
                        <button onClick={()=>setSeatsPerTable(s=>Math.min(12,s+1))} style={{width:32,height:32,borderRadius:8,background:'#e8eaf6',color:'#5c6bc0',fontSize:18,fontWeight:700}}>＋</button>
                      </div>
                      <div style={{fontSize:10,color:'#b0bec5',marginTop:4}}>人/テーブル</div>
                    </div>
                  </div>
                )}

                {/* 合計席数チェック */}
                <div style={{padding:'10px 14px',borderRadius:10,background:presetOk?'#e8f5e9':'#fff3e0',border:`1.5px solid ${presetOk?'#a5d6a7':'#ffcc80'}`,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <span style={{fontSize:13,color:presetOk?'#2e7d32':'#e65100',fontWeight:700}}>
                    {presetOk ? '✅ 席数OK！' : tableType==='u' ? `コの字は全体で${people}人に自動調整` : `合計 ${totalPresetSeats}席 ≠ ${people}人`}
                  </span>
                  {!presetOk && tableType!=='u' && (
                    <div style={{display:'flex',gap:6}}>
                      <button onClick={()=>{const s=Math.round(people/tableCount);setSeatsPerTable(Math.max(2,s));}} style={{padding:'4px 10px',borderRadius:8,fontSize:11,fontWeight:700,background:'#5c6bc0',color:'#fff',border:'none'}}>自動調整</button>
                    </div>
                  )}
                </div>
              </div>
          ...
          {(previewSeats.length>0) && (
            <div style={C.card}>
              ...
              <div style={{textAlign:'center',fontSize:11,color:'#90a4ae',marginTop:6}}>
                {previewTables.length}テーブル × {layoutMode==='preset'?seatsPerTable:'?'}席 ＝ {previewSeats.length}席
              </div>
            </div>
          )}
          ...
          <button onClick={startRoulette} disabled={!canStart}
            ...
            {!canStart?(layoutMode==='preset'?`⚠️ テーブル数×人数を${people}人に合わせてください`:`⚠️ あと ${people-totalCustomSeats} 席追加`):'🎲 スタート！'}
          </button>
        </div>
      )}
